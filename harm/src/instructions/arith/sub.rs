...
pub fn sub<T, Src1, Src2>(
    dst: T,
    src1: Src1,
    src2: Src2,
) -> <Sub<T, Src1, Src2> as MakeSub<T, Src1, Src2>>::Output
where
...
";
    ...
    test_cases! {
        ...
        test_sub_64_zero,
            sub(X1.into(), XZR, ShiftedReg::from(X12).shift(ShiftMode::LSR, 4)),
            "sub x1, xzr, x12, lsr #4";
        test_sub_64_extend_uxtx, sub(RegS(X1), RegS(X2), RegZ(X12)).extend(ExtendMode::UXTX, 3),
        ...
        test_sub_64_wzr_extend_uxtw, sub(RegS(X1), RegS(X2), XZR).extend(ExtendMode::UXTW, 3), "sub x1, x2, wzr, uxtw #3";
        ...
        test_sub_32_zero,
            sub(W1.into(), WZR, ShiftedReg::from(W12).shift(ShiftMode::LSR, 4)),
        ...
        test_sub_32_extend_uxtx_wzr,
            sub(Reg3S(W1), Reg3S(W2), WZR).extend(ExtendMode::UXTX, 3),
            "sub w1, w2, wzr, uxtx #3";
        test_sub_32_extend_uxtw_wzr,
            sub(Reg3S(W1), Reg3S(W2), WZR).extend(ExtendMode::UXTW, 3),
